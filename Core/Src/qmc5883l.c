...
HAL_StatusTypeDef qmc5883l_read(qmc5883l_t *dev, float *mx, float *my, float *mz)
{
    ...
    *mz = (float)zr / dev->lsb_per_ut;
    ...
    /* ---------- Монтаж кристалла: оси чипа -> оси платы.
     * QMC5883L/HA5883 на GY-273 (GY-270) установлен повёрнутым на 180°
     * вокруг оси X платы (вперёд) относительно MPU6050: его Y смотрит
     * вправо, Z — вниз. Подтверждено сырыми показаниями магнитомера в
     * трёх позах (лежит/нос вверх/нос вниз, 2026-09): без этой правки
     * азимут зеркален по запад/восток и прыгает на ~180° при тангаже.
     * HMC5883L (GY-271): монтаж не проверялся — оси передаются как есть;
     * если на конкретной плате азимут зеркален, поменяйте знаки здесь. */
    if (dev->type == MAG5883_QMC) {
        const float bx = *mx;
        const float by = -*my;
        const float bz = -*mz;
