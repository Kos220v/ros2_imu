...
Датчик угла на основе гироскопа и акселерометра **MPU6050/MPU6500**
(клон, тот же I2C-модуль) + магнитометра
**QMC5883L** (маркировка HA5883/DB5883) или **HMC5883L** — определяется
автоматически. Плата STM32F303 вычисляет **кватернион ориентации** и
**азимут**, робот забирает их по UART в ROS 2.
...
```bash
./tests/run_host_tests.sh   # C-тесты (164 проверки) + Python-тесты протокола
```
